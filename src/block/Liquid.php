...
abstract class Liquid extends Transparent{
	...
	private function onFlowUpdate() : void{
		...
		if(!$this->isSource()){
			...
			if($minAdjacentSources !== null && $this->adjacentSources >= $minAdjacentSources){
				...
				if($bottomBlock->isSolid() || ($bottomLiquid instanceof Liquid && $bottomLiquid->hasSameTypeId($this) && $bottomLiquid->isSource())){
					$newDecay = 0;
					$falling = false;
				}
			}
	...
	protected function flowIntoBlock(Block $block, int $newFlowDecay, bool $falling) : void{
		if($this->canFlowInto($block) && !($block instanceof Liquid)){
			...
			if(!$ev->isCancelled()){
				$world = $this->position->getWorld();
				$waterloggedResult = $block->canBeWaterlogged() && $ev->getNewState()->canBeWaterlogged() && $ev->getNewState()->hasSameTypeId($block);
				if(!$waterloggedResult && $block->getTypeId() !== BlockTypeIds::AIR){
