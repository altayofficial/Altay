...
abstract class Liquid extends Transparent{
	...
	protected function getEffectiveFlowDecay(Block $block) : int{
		$liquid = $block instanceof Liquid ? $block : $block->getWaterlogging();
		if($liquid === null || !$liquid->hasSameTypeId($this)){
			return -1;
		}

		return $liquid->falling ? 0 : $liquid->decay;
	}
	...
	public function onScheduledUpdate() : void{
		...
		$waterlogging = $owningBlock->getWaterlogging();
		$isWaterlogging = !($owningBlock instanceof Liquid) && $waterlogging instanceof Liquid && $waterlogging->hasSameTypeId($this);
	...
	protected function flowIntoBlock(Block $block, int $newFlowDecay, bool $falling) : void{
		if($this->canFlowInto($block) && !($block instanceof Liquid)){
			...
			if(!$ev->isCancelled()){
				...
				if($newState instanceof Liquid && $block->canContainLiquid($newState) && $block->canBeWaterloggedByFlowingLiquid($newState)){
					$world->setLiquid($block->position, $newState);
				}elseif($block->getTypeId() !== BlockTypeIds::AIR){
					$world->useBreakOn($block->position);
					$world->setBlock($block->position, $newState);
				}else{
					$world->setBlock($block->position, $newState);
				}
			}
	...
	protected function canFlowInto(Block $block) : bool{
		$waterlogging = $block->getWaterlogging();
		return
			$this->position->getWorld()->isInWorld($block->position->x, $block->position->y, $block->position->z) &&
			($block->canBeFlowedInto() || ($block->canContainLiquid($this) && $block->canBeWaterloggedByFlowingLiquid($this) && $waterlogging === null)) &&
			!($block instanceof Liquid && $block->isSource()) &&
			!($waterlogging instanceof Liquid && $waterlogging->isSource()); //TODO: I think this should only be liquids of the same type
	}

	protected function isSideAvailable(Block $block, int $face) : bool{
		return !$block instanceof Waterloggable || $block->isSideOpenToFlow($face);
	}
}
